...
$hour = date('H');
$min = date('i');
$result = mysqli_query($conn, 'SELECT `user_id`, `type`, `tokens`, `end`, `custom`, `limit_react` FROM vipreaction ORDER BY RAND()');
while ($vip = mysqli_fetch_assoc($result)) {
    if ($vip['end'] >= time()) { // neu con han
        if($hour == 00 && $min < 10){
            ...
            $check = json_decode(cURL('https://graph.fb.me/me?access_token=' . $vip['tokens'] . '&fields=id&method=get'), true);
            if (isset($check['id'])) { // neu token live
                ...
                // lay list stt
                $home = json_decode(cURL('https://graph.facebook.com/me/home?limit=2&fields=id,from&access_token=' . $vip['tokens'] . '&method=get'), true);
                ...
                foreach ($home['data'] as $act) { // duyet new feed
                    if ($vip['custom'] == 0) { // neu chi bot stt ban be
                        if (!isset($act['from']['category']) && strpos($listid, $act['id']) === false) { // loc & check trung
                            $reaction = json_decode(cURL('https://graph.fb.me/' . $act['id'] . '/reactions?access_token=' . $vip['tokens'] . '&type=' . $type . '&method=post'), true); // tha cam xuc
                            if ($reaction['success']) {
                        ...
                        if (strpos($listid, $act['id']) === false) {
                            $reaction = json_decode(cURL('https://graph.facebook.com/' . $act['id'] . '/reactions?access_token=' . $vip['tokens'] . '&type=' . $type . '&method=post'), true);
                            if ($reaction['success']) {
                                fwrite($fp, $act['id'] . "\n"); // ghi id da bot vao file
                            }
                        }
