...
if(isset($_GET['id'])){
    $id = $_GET['id'];
    $get = "SELECT user_name, email, profile, name, phone,rule,id_agency FROM ctv WHERE id_ctvs=$id";
    ...
    if(isset($_POST['submit'])){
        ...
        if(isset($_POST['freelancer'])){
            $update = "UPDATE ctv SET name='$name', profile='$profile', phone='$sdt',id_agency = -1 WHERE id_ctvs = $id";
        }else{
            $update = "UPDATE ctv SET name='$name', profile='$profile', phone='$sdt' WHERE id_ctvs = $id";
        }
        if(mysqli_query($conn, $update)){
            ...
            if(mysqli_query($conn, $his)){
             echo "<script>alert('Cập nhật thông tin thành công'); window.location='cong-tac-vien.html';</script>";
            }
        }
    }
}
