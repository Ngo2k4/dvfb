...
if (isset($_GET['id_like'])) {
    ...
    if ($rule != 'admin' && $rule !='agency') {
        if ($ctv != $idctv) {
            ...
            echo "<script>alert('ID này vẫn chưa hết hạn'); window.location='ctv-vipid-cam-xuc.html';</script>";
        } else {
        ...
        if ($end > time()) {
            echo "<script>alert('ID này vẫn chưa hết hạn'); window.location='index.php?DS=CTV_Like';</script>";
        }else{
            $sql = "DELETE FROM vip WHERE id = $id_like";
            if (mysqli_query($conn, $sql)) {
                $up = "UPDATE ctv SET num_id = num_id - 1 WHERE id_ctvs=$ctv";
