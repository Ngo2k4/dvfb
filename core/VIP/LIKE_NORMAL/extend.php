    ...
    if ((($_POST['han'] != 'one' && $_POST['han'] != 'three') && $_POST['han'] !='seven') && (($_POST['han'] <= 0 || $_POST['han'] > 12) || $_POST['goi'] < 15000)) {
       ...
       if ($han == 'seven') {
        ...
        if($likes <= $max_like){
            ...
            echo "<script>alert('Vui lòng chọn số Likes/Cron <= Gói Like'); window.location='vip-cam-xuc.html';</script>";
        }
    }
}
?>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <select id="goi" name="goi" class="form-control" onchange="tinh()" <?php if($rule == 'admin' || $rule == 'agency') echo 'disabled'; ?>>
                                <?php
                                $ds = "SELECT max, price FROM package WHERE type='LIKE' AND max <= 600 ORDER BY price ASC";
